...
const PKCE = { verifier: "v".repeat(43), challenge: "chal", method: "S256" };
const tokenResponse = {
    access_token: "at-1",
    refresh_token: "rt-1",
    id_token: "it-1",
...
describe("exchangeCode", () => {
    ...
    it("rejects a response missing a refresh_token instead of typing undefined as string", async () => {
        ...
        await expect(exchangeCode(CONFIG, "code-1", PKCE, fetchMock)).rejects.toBeInstanceOf(AuthError);
    });
...
describe("revokeToken", () => {
    it("posts the token to the revoke endpoint", async () => {
        ...
        expect(new URLSearchParams(init.body).get("token")).toBe("at-1");
    });
});
