...
export function createClient(config, store, doFetch, hooks = {}) {
    /**
    ...
    const refreshOnce = (stale) => {
        ...
        inFlightRefresh = (async () => {
            try {
                ...
                if (current && current.accessToken !== stale.accessToken)
                    return current;
    ...
    const requireTokens = async () => {
        const tokens = await store.load();
        if (!tokens)
            throw new SessionExpiredError("no session on device");
        return isExpired(tokens, Date.now()) ? refreshOnce(tokens) : tokens;
    };
    const send = async (method, body, tokens) => doFetch(`${config.baseUrl}/api/method/${method}`, {
        method: "POST",
        headers: {
    ...
    const call = async (method, body) => {
        ...
        if (!response.ok) {
            throw new FrappeApiError(payload.exception ?? `${method} failed with ${response.status}`, response.status, parseServerMessages(payload._server_messages));
        }
