...
const parent = (over: Partial<OutboxRow> = {}): OutboxRow => ({
	id: 1,
...
const queued = (over: Partial<PendingUpload> = {}): PendingUpload => ({
	id: 10,
...
const repo = (rows: OutboxRow[], uploads: PendingUpload[]) => ({
...
describe("drainUploads", () => {
	it("attaches to the document the parent write acted on", async () => {
		...
		await drainUploads(store as never, call as never, reader());

		expect(call).toHaveBeenCalledWith(
			expect.stringContaining("attach_mobile_file"),
	...
	it("deletes the on-device copy once the server confirms", async () => {
		...
		await drainUploads(store as never, (async () => ({ data: { status: "Applied" } })) as never, files);
	...
	it("keeps the photo when the upload fails", async () => {
		...
		const outcomes = await drainUploads(store as never, call as never, files);
	...
	it("treats a server-side Duplicate as done", async () => {
		...
		const outcomes = await drainUploads(
			store as never,
			(async () => ({ data: { status: "Duplicate" } })) as never,
			files,
		);
