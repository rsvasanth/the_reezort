...
function toRow(db) {
    return {
        ...
        action: db.action,
        payload: JSON.parse(db.payload_json),
        ...
        conflict: db.conflict_json ? JSON.parse(db.conflict_json) : undefined,
...
export function createOutboxRepository(sql) {
    const persist = async (row) => {
        await sql.run(`UPDATE outbox SET client_request_id = ?, base_modified = ?, state = ?, attempts = ?,
    ...
    const byClientRequestId = async (id) => {
        const found = await sql.all(`SELECT ${COLUMNS} FROM outbox WHERE client_request_id = ?`, [id]);
        return found[0] ? toRow(found[0]) : null;
    };
    const byId = async (id) => {
        const found = await sql.all(`SELECT ${COLUMNS} FROM outbox WHERE id = ?`, [id]);
        return found[0] ? toRow(found[0]) : null;
    };
    return {
        async init() {
        ...
        async enqueue(input) {
            await sql.run(`INSERT INTO outbox
            ...
            if (!row)
                throw new Error(`Enqueue of ${input.clientRequestId} did not persist`);
            return row;
        },
        async list() {
            // Ascending id is drain order — the order the attendant acted in.
            const rows = await sql.all(`SELECT ${COLUMNS} FROM outbox ORDER BY id ASC`);
         ...
         */
        async markInFlight(ids, leaseUntil) {
            for (const id of ids) {
        ...
        async applyResults(results, now) {
        ...
        async resolveKeepMine(id, newClientRequestId) {
            const row = await byId(id);
            if (!row)
                throw new Error(`No outbox row ${id}`);
        ...
        async queueUpload(outboxId, localUri, contentHash, clientRequestId) {
            await sql.run(`INSERT INTO pending_upload (client_request_id, outbox_id, local_uri, content_hash)
				 VALUES (?, ?, ?, ?)`, [clientRequestId, outboxId, localUri, contentHash]);
        },
        /** Every queued photo, for the drain worker to filter. */
        async listUploads() {
            const rows = await sql.all(`SELECT id, client_request_id, outbox_id, local_uri, content_hash, state, attempts
        ...
        async markUploadState(id, state, bumpAttempts = false) {
            await sql.run(`UPDATE pending_upload SET state = ?, attempts = attempts + ? WHERE id = ?`, [state, bumpAttempts ? 1 : 0, id]);
        },
        /** Called once the server confirms the attachment. */
        async deleteUpload(id) {
            await sql.run("DELETE FROM pending_upload WHERE id = ?", [id]);
        },
        async uploadsFor(outboxId) {
            const rows = await sql.all(`SELECT id, client_request_id, outbox_id, local_uri, content_hash, state, attempts
				 FROM pending_upload WHERE outbox_id = ? ORDER BY id ASC`, [outboxId]);
            return rows.map((r) => ({
                id: r.id,
        ...
        async summary() {
            const rows = await sql.all("SELECT state, COUNT(*) AS n FROM outbox GROUP BY state");
            const count = (...states) => rows.filter((r) => states.includes(r.state)).reduce((sum, r) => sum + r.n, 0);
            return {
