 ...
 */
function nodeSqlite(options: { swallowPragma?: boolean } = {}): SqlExecutor {
...
let repo: ReturnType<typeof createOutboxRepository>;
...
describe("applyResults", () => {
	...
	it("ignores a result for a row this device does not have", async () => {
		...
		await expect(
			repo.applyResults([{ clientRequestId: "unknown", status: "Applied" }], 0),
...
describe("resolveKeepMine", () => {
	...
	it("lands in refused, not failed, when the re-push is rejected", async () => {
		const row = await repo.enqueue({ ...input, clientRequestId: "req-original", createdAt: 1 });
		await repo.applyResults(
			[{ clientRequestId: "req-original", status: "Conflict", serverModified: "s" }],
			0,
		);
		await repo.resolveKeepMine(row.id, () => "req-fresh");

		await repo.applyResults(
			[{ clientRequestId: "req-fresh", status: "Rejected", errorMessage: "Room is occupied" }],
			0,
		);
