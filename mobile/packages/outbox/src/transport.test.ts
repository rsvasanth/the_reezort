...
import { drainOnce, fromWireResult, toWireOperation, type WireResult } from "./transport";
...
describe("fromWireResult", () => {
	...
	it("leaves a Duplicate of a genuine success as applied", () => {
		expect(
			fromWireResult({ client_request_id: "r", status: "Duplicate", original_status: "Applied" })
				.status,
		).toBe("Applied");
	});
...
describe("drainOnce", () => {
	...
	it("unwraps the house envelope before reading results", async () => {
		...
		expect(store.applyResults).toHaveBeenCalledWith(
			[expect.objectContaining({ clientRequestId: "req-1", status: "Applied" })],
			0,
		);
	});
