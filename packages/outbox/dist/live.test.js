...
const b64url = (raw) => raw.toString("base64url");
...
async function bearerToken(clientId) {
    const s = session();
    ...
    const redirect = await s(`${BENCH}/api/method/frappe.integrations.oauth2.authorize?${params}`);
    const location = redirect.headers.get("location") ?? "";
    const code = new URL(location).searchParams.get("code");
    if (!code)
        throw new Error(`No authorization code. Location: ${location}`);
    ...
    if (!token.access_token)
        throw new Error(`No token: ${JSON.stringify(token)}`);
...
async function adminCall() {
    ...
    return async (path, init = {}) => (await s(`${BENCH}${path}`, init)).json();
}
function nodeSqlite() {
...
live("outbox against a real bench", () => {
    ...
    let taskModified;
    beforeAll(async () => {
        admin = (await adminCall());
        const clients = await admin(`/api/resource/OAuth Client?filters=[["app_name","=","reezort-ops"]]&fields=["client_id"]`);
        const clientId = clients.data[0]?.client_id;
        if (!clientId)
            throw new Error("No reezort-ops OAuth Client — run seed_mobile on the bench");
        ...
        const rooms = (await admin(`/api/resource/Room?fields=["name"]&limit_page_length=1`));
        const room = rooms.data[0]?.name;
        if (!room)
            throw new Error("No Room on the bench to attach a task to");
        const created = await admin("/api/method/the_reezort.housekeeping.api.create_task", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                payload: {
        ...
        taskName = created.message.data.task
            .name;
        ...
        const doc = await admin(`/api/resource/Housekeeping Task/${taskName}`);
        taskModified = doc.data.modified;
    }, 60_000);
